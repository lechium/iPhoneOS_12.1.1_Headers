/*
* This header is generated by classdump-dyld 1.0
* on Saturday, June 1, 2019 at 6:52:45 PM Mountain Standard Time
* Operating System: Version 12.1.1 (Build 16C5050a)
* Image Source: /System/Library/PrivateFrameworks/News/Tea.framework/Tea
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Tea/Tea-Structs.h>
#import <UIKitCore/UIViewController.h>
#import <libobjc.A.dylib/TSTabBarSplitViewAutoObserver.h>
#import <libobjc.A.dylib/TSTabBarSplitViewPresentable.h>

@class NSString;

@interface Tea.TableOfContentsContainerViewController : UIViewController <TSTabBarSplitViewAutoObserver, TSTabBarSplitViewPresentable> {

	 activeNavigationBarHeightManager;
	 contentViewController;
	 navigationItemManager;

}

@property (readonly,nonatomic) NSString * tabBarSplitViewIdentifier; 
@property (readonly,nonatomic) BOOL shouldAutomaticallyForwardAppearanceMethods; 
-(void)tabBarSplitViewControllerWillCollapseWithDisplayMode:(long long)arg1 ;
-(void)tabBarSplitViewControllerWillExpandWithDisplayMode:(long long)arg1 ;
-(void)tabBarSplitViewWillChangeDisplayModeTo:(long long)arg1 ;
-(void)tabBarSplitViewControllerDidAcquireInitialState;
-(void)traitCollectionDidChange:(id)arg1 ;
-(void)viewWillAppear:(BOOL)arg1 ;
-(void)viewWillDisappear:(BOOL)arg1 ;
-(void)viewDidLoad;
-(void)viewWillTransitionToSize:(CGSize)arg1 withTransitionCoordinator:(id)arg2 ;
-(void)viewDidLayoutSubviews;
-(void)viewDidAppear:(BOOL)arg1 ;
-(void)viewDidDisappear:(BOOL)arg1 ;
-(id)tabBarSplitViewPresentationFor:(long long)arg1 ;
-(NSString *)tabBarSplitViewIdentifier;
-(id)initWithCoder:(id)arg1 ;
-(id)initWithNibName:(id)arg1 bundle:(id)arg2 ;
-(BOOL)shouldAutomaticallyForwardAppearanceMethods;
@end

