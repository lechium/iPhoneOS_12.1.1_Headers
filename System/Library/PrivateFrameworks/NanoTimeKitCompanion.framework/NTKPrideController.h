/*
* This header is generated by classdump-dyld 1.0
* on Saturday, June 1, 2019 at 6:51:26 PM Mountain Standard Time
* Operating System: Version 12.1.1 (Build 16C5050a)
* Image Source: /System/Library/PrivateFrameworks/NanoTimeKitCompanion.framework/NanoTimeKitCompanion
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class NTKPrideGraphDataSource, NTKPrideListing;

@interface NTKPrideController : NSObject {

	NTKPrideGraphDataSource* _dataSource;
	NTKPrideListing* _currentListing;
	NTKPrideListing* _preparedListing;

}

@property (nonatomic,retain) NTKPrideListing * currentListing;               //@synthesize currentListing=_currentListing - In the implementation block
@property (nonatomic,retain) NTKPrideListing * preparedListing;              //@synthesize preparedListing=_preparedListing - In the implementation block
-(id)initForDevice:(id)arg1 ;
-(NTKPrideListing *)currentListing;
-(void)setCurrentListing:(NTKPrideListing *)arg1 ;
-(void)proceedToNextListing;
-(id)getListingWithTag:(id)arg1 ;
-(void)proceedToNextListingWithTag:(id)arg1 ;
-(NTKPrideListing *)preparedListing;
-(void)prepareUserTapAction;
-(void)invalidateCurrentListing;
-(void)startedPlayingListing:(id)arg1 ;
-(id)currentPosterImage;
-(id)currentComplicationColor;
-(id)currentStyleComplicationColor;
-(void)setPreparedListing:(NTKPrideListing *)arg1 ;
@end

